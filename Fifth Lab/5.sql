/*
	...
	входного параметра id заказа 
	(Purchasing.PurchaseOrderHeader.PurchaseOrderID) и возвращать сумму 
	по заказу из детализированного списка заказов 
	(Purchasing.PurchaseOrderDetail.LineTotal).

	Создайте inline table-valued функцию, которая будет принимать в 
	качестве входных параметров id заказчика (Sales.Customer.CustomerID)
	и количество строк, которые необходимо вывести.

	Функция должна возвращать определенное количество самых прибыльных 
	заказов (по TotalDue) из Sales.SalesOrderHeader для каждого заказчика.
	...
	table-valued (предварительно сохранив для проверки код создания inline 
